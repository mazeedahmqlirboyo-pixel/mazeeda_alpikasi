...
// 3. Fix image source (kalender_bg.png -> kalender_premium.png)
content = content.replace(
...
// 5. Fix Dropdown active/hover states in dark mode
content = content.replace(
...
// 6. Fix filterPeringatan Syawal bug (split by " - " instead of space)
content = content.replace(
  /const hMonths = dominantHijriMonth\.toLowerCase\(\)\.split\(\/\[ -\]\+\/\);\s*for \(let hm of hMonths\) \{\s*if \(hm\.length > 3 && t\.includes\(hm\)\) return true;\s*if \(hm === "dzulhijjah" && t\.includes\("zulhijah"\)\) return true;\s*if \(hm === "muharram" && t\.includes\("muharam"\)\) return true;\s*\}/,
  \const hMonths = dominantHijriMonth.toLowerCase().split(" - ").map(s => s.trim());
      for (let hm of hMonths) {
        ...
        if (tClean.split(/\s+/).includes(hmClean)) return true;
        if (hmClean === "dzulhijjah" && tClean.includes("zulhijah")) return true;
        if (hmClean === "muharram" && tClean.includes("muharam")) return true;
      }\
);

// 7. Implement Tampilkan Semua / Sembunyikan toggle
content = content.replace(
...
// reset toggle when switching dropdowns
content = content.replace(
...
console.log('All changes applied cleanly via Node.js script.');
