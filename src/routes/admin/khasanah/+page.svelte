...
<div class="min-h-screen bg-slate-50 dark:bg-slate-900 pb-24 font-sans">
  {#if alertMessage}
  ...
  <div class="max-w-5xl mx-auto px-4 py-8 space-y-6">
    <div class="flex items-center space-x-4 mb-8">
      <a href="/admin">
        <Button variant="outline" class="p-2 border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:bg-slate-800">
          <ArrowLeft class="h-5 w-5 text-slate-600 dark:text-slate-300" />
        </Button>
      </a>
      <div>
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Khasanah Lirboyo CMS</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">Kelola artikel dinamis dengan Rich Text Editor.</p>
      </div>
    </div>
    ...
    {#if !showForm}
      <Card class="p-6">
        <div class="flex justify-between items-center mb-6">
          <h2 class="text-lg font-bold text-slate-800 dark:text-slate-100">Daftar {tabs.find(t => t.value === activeTab)?.label}</h2>
        ...
        {#if isLoading}
          <div class="flex justify-center p-8 text-slate-400 dark:text-slate-500">
        ...
        {:else}
          <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
              <thead>
                <tr class="bg-slate-50 dark:bg-slate-800 border-y border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 dark:text-slate-500 text-xs uppercase tracking-wider">
              ...
              <tbody class="divide-y divide-slate-100 text-sm">
                {#each items as item}
                  <tr class="hover:bg-slate-50 dark:bg-slate-800 transition-colors">
                    <td class="px-4 py-3">
                      <img src={item.image_url} alt="" class="w-12 h-12 rounded object-cover border border-slate-200 dark:border-slate-700" />
                    </td>
                    <td class="px-4 py-3">
                      <p class="font-bold text-slate-800 dark:text-slate-100">{item.name || item.author || item.title}</p>
                      <p class="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">{item.title !== (item.name || item.author || item.title) ? item.title : '' || item.type}</p>
                    </td>
                ...
                {:else}
                  <tr><td colspan="4" class="text-center py-8 text-slate-400 dark:text-slate-500 text-sm">Tidak ada data.</td></tr>
                {/each}
              </tbody>
            </table>
          </div>
    ...
    {:else}
      <Card class="p-6 animate-in slide-in-from-bottom-4 duration-300">
        <div class="flex justify-between items-center mb-6 border-b border-slate-100 dark:border-slate-800 pb-4">
          <h2 class="text-lg font-bold text-slate-800 dark:text-slate-100">{editingId ? 'Edit Data' : 'Tambah Baru'}</h2>
          <Button variant="outline" on:click={resetForm} class="h-8 border-slate-200 dark:border-slate-700">Batal</Button>
        </div>
        ...
        <form on:submit|preventDefault={saveItem} class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            {#if activeTab !== 'sambutan'}
              <div class="space-y-1">
                <label class="text-xs font-semibold text-slate-600 dark:text-slate-300">Slug (URL)</label>
                <Input bind:value={formSlug} placeholder="contoh: kh-ahmad-fulan" required />
              </div>
            {/if}

            <div class="space-y-1">
              <label class="text-xs font-semibold text-slate-600 dark:text-slate-300">{activeTab === 'filosofi' ? 'Nama Logo' : activeTab === 'sambutan' ? 'Pemateri' : 'Nama Lengkap'}</label>
              <Input bind:value={formName} required />
            </div>

            {#if activeTab !== 'filosofi'}
            <div class="space-y-1">
              <label class="text-xs font-semibold text-slate-600 dark:text-slate-300">{activeTab === 'units' ? 'Tipe Unit' : 'Gelar / Judul'}</label>
            ...
            <div class="space-y-1">
              <label class="text-xs font-semibold text-slate-600 dark:text-slate-300">
                Link Foto (Image URL)
                <span class="block font-normal text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">
              ...
              <div class="flex space-x-2">
                ...
                <Button type="button" variant="outline" on:click={() => fileInputImage.click()} disabled={isUploadingImg} class="px-3 border-slate-200 dark:border-slate-700" title="Upload dari perangkat">
            ...
            {#if activeTab === 'sambutan'}
              <div class="space-y-1">
                <label class="text-xs font-semibold text-slate-600 dark:text-slate-300">
                  ...
                  <span class="block font-normal text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">Rekomendasi: Gambar PNG Transparan (Maks. Tinggi 200px)</span>
                </label>
                <div class="flex space-x-2">
                  ...
                  <Button type="button" variant="outline" on:click={() => fileInputSignature.click()} disabled={isUploadingSig} class="px-3 border-slate-200 dark:border-slate-700" title="Upload dari perangkat">
          ...
          <div class="space-y-1">
            <label class="text-xs font-semibold text-slate-600 dark:text-slate-300">{activeTab === 'sambutan' ? 'Kutipan Pendek' : 'Deskripsi Singkat'}</label>
            <textarea bind:value={formShortDesc} class="w-full min-h-[80px] p-3 text-sm rounded-lg border border-slate-200 dark:border-slate-700 focus:border-primary focus:ring-1 focus:ring-primary outline-none" required></textarea>
          </div>

          <div class="space-y-1">
            <label class="text-xs font-semibold text-slate-600 dark:text-slate-300 flex items-center justify-between">
              Isi Artikel Lengkap
