<script lang="ts">
  ...
  let adminCommentsSearch = '';
  
  $: filteredAdminComments = adminCommentsList.filter(c => {
    if (!adminCommentsSearch.trim()) return true;
    const search = adminCommentsSearch.toLowerCase();
    const author = (c.user_name || c.author || 'Anonim').toLowerCase();
    const text = (c.comment_text || c.text || c.message || '').toLowerCase();
  ...
  // User Reports State
  let userReportsList: any[] = [];
  let isLoadingUserReports = false;
  let userReportsFilter = 'pending'; // 'pending', 'resolved', 'rejected', 'all'
  ...
  async function fetchUserReports() {
    ...
    try {
      ...
      if (userReportsFilter !== 'all') {
        query = query.eq('status', userReportsFilter);
      }
      const { data, error } = await query.limit(100);
      if (error) throw error;
      userReportsList = data || [];
    } catch (e) {
      console.error(e);
      triggerAlert('Gagal memuat laporan pengguna', 'error');
    } finally {
  ...
  async function executeReportAction(reportId: string, reportedName: string, actionType: 'deactivate' | 'delete') {
    runWithConfirmation(
      actionType === 'deactivate' ? 'Nonaktifkan Akun' : 'Hapus Akun',
      `Tindakan ini akan ${actionType === 'deactivate' ? 'menonaktifkan' : 'menghapus secara permanen'} akun ${reportedName}. Lanjutkan?`,
      async () => {
        try {
          const { data: alumni } = await supabase.from('allowed_alumni').select('id, nama_lengkap').ilike('nama_lengkap', reportedName).limit(1);
          const { data: asatidzah } = await supabase.from('asatidzah').select('id, nama_lengkap').ilike('nama_lengkap', reportedName).limit(1);
          
          let targetTable = '';
          let targetId = '';
          
          if (alumni && alumni.length > 0) {
            targetTable = 'allowed_alumni';
            targetId = alumni[0].id;
          } else if (asatidzah && asatidzah.length > 0) {
            targetTable = 'asatidzah';
            targetId = asatidzah[0].id;
          }
          
          if (!targetTable) {
            triggerAlert('Akun tidak ditemukan di database.', 'error');
            return;
          }
          
          if (actionType === 'deactivate') {
            const { error } = await supabase.from(targetTable).update({ is_active: false }).eq('id', targetId);
            if (error) throw error;
            triggerAlert('Akun berhasil dinonaktifkan.');
          } else {
            const { error } = await supabase.from(targetTable).delete().eq('id', targetId);
            if (error) throw error;
            triggerAlert('Akun berhasil dihapus permanen.');
          }
          
          await updateReportStatus(reportId, 'resolved');
        } catch (err: any) {
          triggerAlert('Gagal mengeksekusi aksi: ' + err.message, 'error');
        }
      }
    );
  }
      ...
      <div class="bg-white rounded-3xl shadow-soft-sm border border-slate-100 overflow-hidden">
        ...
        {:else}
          <div class="divide-y divide-slate-100 max-h-[650px] overflow-y-auto custom-scrollbar pr-1">
            {#each filteredAdminComments as comment (comment.id)}
              ...
              <div class="p-5 hover:bg-slate-50/80 transition-all duration-300 group flex flex-col gap-3 relative overflow-hidden">
                ...
                {#if report.status === 'pending'}
                  <div class="flex gap-2 justify-end mt-2">
                    ...
                    <button 
                      on:click={() => executeReportAction(report.id, report.reported_name, 'deactivate')}
                      class="px-3 py-1.5 bg-orange-100 text-orange-700 hover:bg-orange-200 rounded-lg text-xs font-bold transition-colors flex items-center gap-1"
                    >
                      <UserX class="w-3.5 h-3.5" /> Nonaktifkan
                    </button>
                    <button 
