<script lang="ts">
  ...
  import { ScrollText, Loader2 } from 'lucide-svelte';
  import { supabase } from '$lib/supabase';
  import { defaultTermsConditions } from '$lib/data/defaultLegal';
  ...
  onMount(async () => {
    try {
      ...
      if (data) {
        ...
        content = defaultTermsConditions;
      ...
      <div class="bg-white rounded-[2rem] p-6 md:p-10 shadow-sm border border-slate-100 min-h-[300px] flex flex-col">
        {#if isLoading}
          <div class="flex flex-col items-center justify-center flex-1 text-slate-400 space-y-3">
            ...
            <p class="text-sm font-medium animate-pulse">Memuat persyaratan...</p>
          </div>
        {:else}
          <div class="space-y-0">
            {@html content}
