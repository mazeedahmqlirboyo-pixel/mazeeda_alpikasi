<script lang="ts">
  ...
  function handleSliderImageError(e: Event) {
    if (selectedMember && allProfilePhotos.length > 0) {
      if (currentPhotoIndex === 0 && allProfilePhotos[0].type !== 'custom') {
        failedImages.add(selectedMember.id);
  ...
  onMount(async () => {
    try {
      ...
      if (shareId) {
        ...
        const { data: memberData } = await supabase.from('allowed_alumni').select('*').ilike('nama_lengkap', shareName).maybeSingle();
        if (memberData) {
          openDetails(memberData);
        } else { directLinkFailed = true; }
      } else {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        if (error) {
          ...
          if (typeof window !== 'undefined') alert('Gagal memuat foto tambahan: ' + error.message);
        }
  ...
  async function handlePhotoUpload(e: Event) {
    ...
    try {
      const uploadedImage = await uploadCustomProfilePhoto(file, selectedMember.nama_lengkap);
      
      // Update local state
      const finalUrl = typeof uploadedImage === 'string' ? uploadedImage : (uploadedImage as any).url;
      const newPhoto = { url: finalUrl, type: 'custom', status: 'pending' };
  ...
  function closeProfile() {
    ...
    if (from === 'kepengurusan') {
      window.location.href = '/kepengurusan';
    } else {
      selectedMember = null;
      isImageLarge = false;
  ...
  $: isDirectLinkLoading = browser && ($page.url.searchParams.has('id') || $page.url.searchParams.has('name')) && !selectedMember && !directLinkFailed;
...
{#if selectedMember}
  ...
  <div class="space-y-6 animate-in fade-in duration-300">
    ...
    <div class="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800 relative">
      ...
      <!-- Centered Text -->
      <span class="absolute left-1/2 -translate-x-1/2 text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider block whitespace-nowrap">MAZEEDA SQUAD PROFILE</span>
      
      <!-- 3-dots Menu -->
      <div class="relative">
        <button on:click={() => showSquadMenu = !showSquadMenu} class="inline-flex items-center justify-center w-10 h-10 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800/50 text-slate-500 dark:text-slate-400 transition-colors">
        ...
        {#if showSquadMenu}
          ...
          <div class="absolute right-0 top-full mt-1 w-52 bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-slate-100 dark:border-slate-800 overflow-hidden z-30 animate-in fade-in slide-in-from-top-2 py-1">
            ...
            <button 
              ...
              disabled={isBlocking}
              class="w-full text-left px-4 py-2.5 text-sm font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:bg-slate-800 flex items-center gap-2.5 transition-colors disabled:opacity-50"
            >
            ...
            >
              <div class="w-full h-full rounded-2xl overflow-hidden bg-blue-50/50 dark:bg-slate-800 flex items-center justify-center relative group/inner">
                ...
                {#if allProfilePhotos.length > 0}
                  ...
                  {#if allProfilePhotos.length > 1}
                    <button type="button" on:click={(e) => { e.stopPropagation(); prevPhoto(e); }} class="absolute left-1 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-black/80 text-white rounded-full p-1 opacity-100 transition-opacity z-[100] shadow-md dark:shadow-none">
                      <ChevronLeft class="h-4 w-4" />
                    </button>
                    <button type="button" on:click={(e) => { e.stopPropagation(); nextPhoto(e); }} class="absolute right-1 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-black/80 text-white rounded-full p-1 opacity-100 transition-opacity z-[100] shadow-md dark:shadow-none">
          ...
          >
            <div class="flex items-center gap-3">
              ...
              <div class="flex flex-col">
                <span class="text-sm font-bold text-slate-800 dark:text-slate-100">Rekam Jejak Nilai Akademik</span>
                <span class="text-[11px] font-medium text-slate-500 dark:text-slate-400 dark:text-slate-500">Tamrin, Muhafadzoh, Ujian</span>
              </div>
            </div>
            <div class="w-8 h-8 rounded-full bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-400 dark:text-slate-500 group-hover:bg-emerald-500 group-hover:text-white transition-colors">
              <ChevronRight class="h-4 w-4" />
            </div>
          </a>
        </div>
...
{:else}
  ...
  <div class="space-y-6">
    <PageHeader title={$t('squad.title') || 'Mazeeda Squad'} backTo="/" />
    ...
    <div class="space-y-0">
      ...
      <div class="flex items-center space-x-2 relative">
        <div class="relative flex-1">
          ...
          <Input 
            type="text" 
            placeholder={$t('squad.search_placeholder') || 'Cari berdasarkan nama atau domisili...'} 
            ...
            <div class="absolute right-0 top-[calc(100%+8px)] z-20 w-80 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/80 rounded-2xl shadow-lg overflow-hidden animate-in fade-in zoom-in-95 slide-in-from-top-1 duration-150 origin-top-right">
              ...
              <div class="px-3 pt-2 pb-2">
                ...
                <div class="flex flex-wrap gap-1 max-h-40 overflow-y-auto pr-1">
                  ...
                  {#if uniqueDaerah.filter(d => d.toLowerCase().includes((daerahSearch || '').toLowerCase())).length === 0}
                    <p class="text-[10px] text-slate-400 dark:text-slate-500 italic py-1 px-1">{$t('squad.region_not_found') || 'Daerah tidak ditemukan'}</p>
                  {/if}
                </div>
              </div>
              ...
              {#if activeFilterCount > 0}
                <div class="border-t border-slate-100 dark:border-slate-800 px-3 py-2">
                  ...
                  >{$t('squad.reset_filter') || '✕ Reset semua filter'}</button>
                </div>
              {/if}
            </div>
          {/if}
    ...
    {#if isLoading}
      {#if searchQuery.trim().length > 0 || activeCategory !== 'semua' || activeDaerah !== 'semua'}
        <div class="py-24 text-center space-y-4">
          ...
          <p class="text-xs font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-widest">{$t('squad.loading') || 'Mencari Data...'}</p>
        </div>
      {:else}
          ...
          <div class="max-w-xs mx-auto space-y-2">
            {#if searchQuery.trim().length < 2 && activeCategory === 'semua' && activeDaerah === 'semua'}
              <div class="flex justify-center mb-6">
                <img src="/images/islamic-squad.svg" alt={$t('squad.start_search') || 'Mulai Pencarian'} class="h-40 w-auto object-contain drop-shadow-sm dark:shadow-none" />
              </div>
              <p class="text-sm font-bold text-slate-600 dark:text-slate-300">{$t('squad.start_search') || 'Mulai Pencarian'}</p>
              <p class="text-xs text-slate-400 dark:text-slate-500">{$t('squad.start_search_desc') || 'Ketik minimal 2 huruf nama atau domisili untuk mulai mencari data.'}</p>
            {:else}
              <div class="flex justify-center mb-6">
              ...
              <p class="text-sm font-bold text-slate-600 dark:text-slate-300">{$t('squad.no_data') || 'Tidak ada anggota ditemukan'}</p>
              <p class="text-xs text-slate-400 dark:text-slate-500">{$t('squad.change_keyword') || 'Silakan ganti kata kunci pencarian Anda atau periksa filter yang aktif.'}</p>
            {/if}
          </div>
        </div>
      {/if}
...
{#if isImageLarge && selectedMember && allProfilePhotos.length > 0}
  ...
  >
    <div class="relative max-w-3xl w-full flex flex-col items-center justify-center" on:click|stopPropagation>
      
      {#if allProfilePhotos.length > 1}
        <button type="button" on:click={prevPhoto} class="absolute left-2 md:-left-12 top-1/2 -translate-y-1/2 bg-white dark:bg-slate-900/10 hover:bg-white dark:bg-slate-900/20 text-white rounded-full p-3 transition-colors z-[101]">
          <ChevronLeft class="h-6 w-6 md:h-8 md:w-8" />
        </button>
        <button type="button" on:click={nextPhoto} class="absolute right-2 md:-right-12 top-1/2 -translate-y-1/2 bg-white dark:bg-slate-900/10 hover:bg-white dark:bg-slate-900/20 text-white rounded-full p-3 transition-colors z-[101]">
      ...
      <div class="relative overflow-hidden rounded-3xl border border-white/10 shadow-2xl bg-slate-900/40">
        ...
        {#if allProfilePhotos[currentPhotoIndex].status === 'pending'}
          <div class="absolute inset-0 flex flex-col items-center justify-center p-6 text-center z-10 pointer-events-none">
            <div class="bg-amber-500 text-white font-black text-sm md:text-xl px-4 py-2 md:px-6 md:py-3 rounded-2xl shadow-xl transform -rotate-12 border-4 border-white/20 animate-pulse">
        ...
        <div class="absolute bottom-0 inset-x-0 bg-gradient-to-t from-slate-950/90 via-slate-950/40 to-transparent p-4 pt-12 text-white z-20">
...
{#if showToast}
  <div class="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
    <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden max-w-sm w-full border border-slate-100 dark:border-slate-800 animate-in zoom-in-95 duration-300 relative flex flex-col items-center text-center p-6 sm:p-8 space-y-4">
      ...
      <button 
        ...
        class="mt-4 w-full {toastType === 'success' ? 'bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800' : 'bg-rose-600 hover:bg-rose-700 active:bg-rose-800'} text-white font-bold py-3 px-4 rounded-xl transition-all shadow-md dark:shadow-none active:scale-[0.98]"
      >
        Oke, Mengerti
      </button>
    </div>
...
{#if isUploadingPhoto}
  <div class="fixed inset-0 z-[999999] flex flex-col items-center justify-center bg-slate-900/80 backdrop-blur-md animate-in fade-in duration-300 p-4">
    <div class="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-2xl flex flex-col items-center gap-4 animate-bounce-slow">
      <div class="h-12 w-12 border-4 border-indigo-200 border-t-indigo-600 rounded-full animate-spin"></div>
      <div class="text-center">
        <p class="font-black text-slate-800 dark:text-slate-100 text-lg">Mengunggah Foto...</p>
        <p class="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500 font-medium">Mohon tunggu sebentar, sedang diproses.</p>
      </div>
    </div>
  </div>
{/if}
...
{#if showReportModal && reportingMember}
  <div class="fixed inset-0 z-[999999] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
    <div class="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
      <div class="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-rose-50/50">
        ...
        <button on:click={() => showReportModal = false} class="p-2 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:bg-slate-800 rounded-full transition-colors">
          <X class="w-5 h-5" />
        </button>
      </div>
      ...
      <div class="p-6 space-y-5 overflow-y-auto max-h-[65vh]">
        <div>
          <p class="text-sm font-medium text-slate-500 dark:text-slate-400 dark:text-slate-500 mb-1">Pengguna yang dilaporkan:</p>
          <div class="font-bold text-slate-800 dark:text-slate-100 px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-xl">
            {reportingMember.nama_lengkap}
          </div>
        </div>
        
        <div>
          <p class="text-sm font-bold text-slate-700 dark:text-slate-200 mb-3">Pilih Alasan Pelaporan <span class="text-rose-500">*</span></p>
          <div class="grid grid-cols-1 gap-2.5">
            {#each ['Spam atau Iklan', 'Konten Tidak Pantas', 'Ujaran Kebencian', 'Penipuan', 'Lainnya'] as reason}
              <label class="flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all {reportReason === reason ? 'border-rose-500 bg-rose-50 shadow-sm dark:shadow-none' : 'border-slate-200 dark:border-slate-700 hover:border-rose-300 hover:bg-slate-50 dark:bg-slate-800'}">
                <input type="radio" name="reportReason" value={reason} bind:group={reportReason} class="w-4 h-4 text-rose-600 focus:ring-rose-500 border-slate-300 dark:border-slate-600" />
                <span class="text-sm font-medium {reportReason === reason ? 'text-rose-700' : 'text-slate-700 dark:text-slate-200'}">{reason}</span>
              </label>
            {/each}
          </div>
        </div>
        ...
        {#if reportReason === 'Lainnya'}
          <div class="animate-in slide-in-from-top-2 duration-300">
            <p class="text-sm font-bold text-slate-700 dark:text-slate-200 mb-2">Jelaskan Alasan Anda <span class="text-rose-500">*</span></p>
            <textarea
              bind:value={reportOtherReason}
              rows="3"
              class="w-full rounded-xl border-slate-200 dark:border-slate-700 shadow-sm dark:shadow-none focus:border-rose-500 focus:ring-rose-500 text-sm p-3 resize-none bg-slate-50 dark:bg-slate-800 focus:bg-white dark:bg-slate-900 transition-colors"
      ...
      <div class="p-5 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex gap-3 justify-end">
        <button 
          on:click={() => showReportModal = false}
          class="px-5 py-2.5 text-slate-600 dark:text-slate-300 font-bold text-sm hover:bg-slate-200 dark:bg-slate-700 bg-slate-100 dark:bg-slate-800 rounded-xl transition-colors"
        ...
        <button 
          ...
          class="px-5 py-2.5 bg-rose-600 text-white font-bold text-sm hover:bg-rose-700 rounded-xl transition-all shadow-md dark:shadow-none shadow-rose-200 disabled:opacity-50 flex items-center gap-2"
        >
...
{#if memberToBlock}
  <div class="fixed inset-0 z-[999999] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
    <div class="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
      <div class="p-6 text-center space-y-4">
        ...
        <h3 class="font-black text-slate-800 dark:text-slate-100 text-xl">Blokir Pengguna?</h3>
        <p class="text-slate-500 dark:text-slate-400 dark:text-slate-500 text-sm leading-relaxed">
          Apakah Anda yakin ingin memblokir <span class="font-bold text-slate-700 dark:text-slate-200">{memberToBlock.nama_lengkap}</span>? Mereka akan disembunyikan dari Squad dan komentar mereka di Timeline tidak akan terlihat lagi.
        </p>
      </div>
      <div class="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex gap-3 justify-end">
        <button 
        ...
        <button 
          ...
          disabled={isBlocking}
          class="flex-1 py-2.5 bg-orange-500 text-white font-bold text-sm hover:bg-orange-600 rounded-xl transition-all shadow-md dark:shadow-none shadow-orange-200 disabled:opacity-50 flex justify-center items-center gap-2"
        >
          {#if isBlocking}
            <div class="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
          {:else}
